...
public class GlobalSkinAtlasCrawler extends WebCrawler {
			...
			if (imgFilters.matcher(url.toLowerCase()).matches() && page.getParseData() instanceof BinaryParseData) {
				...
				try {
					...
					for (Element image : images) {
						if (Integer.parseInt(image.attr("border")) == 1 && image.attr("src").contains("/lg")) {
							totalImgLinked++;
					...
					// System.out.println(curr.html());
					{
						...
						for (Element linkElm : curr.select("a")) {
							linksJSON.put("http://www.globalskinatlas.com/" + linkElm.attr("href"));
						}
